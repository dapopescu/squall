...
//                        if(_numSentTuples % 100 == 0){
//                            System.out.println("Component " + _ID + " has send " + _numSentTuples + " tuples, "
//                                    + " and pending is " + _pendingTuples);
//                        }
			if(line==null) {
                                if(!_hasReachedEOF){
                                    _hasReachedEOF=true;
			...
			//add additional info for a tuple - tupleMultiplicity for now
			Long tupleMultiplicity = 1L;
			
			applyOperatorsAndSend(tuple, tupleMultiplicity);
		}
		...
		if(!MyUtilities.isAckEveryTuple(_conf)){
			if(_hierarchyPosition == FINAL_COMPONENT){
                            ...
                            if(!_hasSentLastAck){
                                ...
                                _collector.emit(new Values("N/A", lastTuple, "N/A", 1L));
                            }
                        }
		}
	}
	...
	protected void applyOperatorsAndSend(List<String> tuple, Object... tupleInfo){
		// do selection and projection
		if(MyUtilities.isBatchOutputMode(_batchOutputMillis)){
			try {
				_semAgg.acquire();
		...
		if(MyUtilities.isSending(_hierarchyPosition, _batchOutputMillis)){
			...
			if (_operatorChain.getAggregation() != null ||  _operatorChain.getDistinct() != null)
				tupleInfo[0] = 1L;
			tupleSend(tuple, null, tupleInfo);
		}
	}
