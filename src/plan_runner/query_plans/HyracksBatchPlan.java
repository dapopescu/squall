...
public class HyracksBatchPlan {
    private static Logger LOG = Logger.getLogger(HyracksBatchPlan.class);

    private QueryPlan _queryPlan = new QueryPlan();

    private static final IntegerConversion _ic = new IntegerConversion();

    public HyracksBatchPlan(String dataPath, String extension, Map conf){
            //-------------------------------------------------------------------------------------
            ...
            EquiJoinComponent CUSTOMER_ORDERSjoin = new EquiJoinComponent(
                    ...
                    _queryPlan).addOperator(postAgg)
                               ...
                               .setBatchOutputMillis(10);
          /*  OperatorComponent ocagg = new OperatorComponent(CUSTOMER_ORDERSjoin, "OCAGG", _queryPlan)
            							.addOperator(postAgg).setHashIndexes(hashIndexes)
            							.setBatchOutputMillis(100);*/

            //-------------------------------------------------------------------------------------
            AggregateSumOperator agg = new AggregateSumOperator(new ColumnReference(_ic, 1), conf)
                    .setGroupByColumns(Arrays.asList(0));

            OperatorComponent oc = new OperatorComponent(/*ocagg*/CUSTOMER_ORDERSjoin, "COUNTAGG", _queryPlan)
                                        .addOperator(agg)
